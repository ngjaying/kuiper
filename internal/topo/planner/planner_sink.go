...
func isSinkDisabled(props map[string]any) (bool, error) {
	...
	if !ok {
		return false, fmt.Errorf("sink disable must be bool, but found %v", raw)
	}
	return disabled, nil
}
...
func splitSink(tp *topo.Topo, s api.Sink, sinkName string, options *def.RuleOption, sc *node.SinkConf, templates []string, schema map[string]*ast.JsonStreamField) ([]node.TopNode, error) {
	...
	if batchEnabled {
		if isBytesCollector {
			batchWriterOp, err := node.NewBatchWriterOp(tp.GetContext(), fmt.Sprintf("%s_%d_batchWriter", sinkName, index), options, schema, sc, transformOp.IsRawOutput())
			if err != nil {
				return nil, err
			}
			index++
