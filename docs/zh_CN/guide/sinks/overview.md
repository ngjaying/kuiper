...
| disable              | bool: false                        | 是否禁用该 sink 动作。若设置为 true，规则规划器会跳过该 sink，不会构造对应的 sink 流水线。规则中至少需要有一个未禁用的 sink 动作。                                                                                                                                                                                                                                                                     |
...
| dataTemplate         | string: ""                         | [golang 模板](https://golang.org/pkg/html/template)格式字符串，用于指定输出数据格式。`sendSingle=false` 时，模板输入为映射数组；`sendSingle=true` 时，模板逐条应用于每个映射。批量配置不会改变模板输入，数据会先转换再写入批次。模板输出被视为已经编码的数据，用户必须确保其符合配置的 `format`。                                                                                                                                                                                                                   |
...
批量处理不会把累计的记录暴露给 `dataTemplate`。批量模式下，模板必须描述一个批次元素，而不是完整批次；模板不应添加批次外层结构或元素分隔符。对于逐条处理的模板，应设置 `sendSingle=true`，使模板逐条作用于每个映射。模板产生已编码输出后，Writer 不会再次编码，只会添加批量格式所需的结构。对于 JSON 批次，每次模板执行都必须产生一个合法的 JSON 值。模板作者负责保证输出符合配置的 `format`；格式 Writer 不会校验模板输出，因此仅设置 `format=json` 不能保证最终批次一定是合法 JSON。
